...
let getYoutubeVideoData = async (youtubeVideoId) => {
  const response = await request
    ...
    .query({
      key:
        ...
        'change-me-with-a-valid-youtube-key-if-you-need-me',
    }) //used only when adding youtube videos
  ...
  const videoDuration = formatDuration(
    response.body.items[0].contentDetails.duration
  );
  ...
  if (tags) {
    //some youtube videos might not have tags defined
    webpageInfo.tags = tags
      .slice(0, 8)
...
let getStackoverflowQuestionData = async (stackoverflowQuestionId) => {
  const response = await request
    .get(
      `https://api.stackexchange.com/2.2/questions/${stackoverflowQuestionId}`
    )
    .query({ site: 'stackoverflow' })
    .query({
      key:
        process.env.STACK_EXCHANGE_API_KEY ||
        'change-me-with-a-valid-stackexchange-key-if-you-need-me',
    });
