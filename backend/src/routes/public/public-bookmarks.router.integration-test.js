...
let generateBookmark = function (
  ...
  return {
    ...
    language: 'en',
    tags: [verySpecialTag, 'async-await', 'mongoose', 'mongodb'],
    publishedOn: '2017-11-05',
    sourceCodeURL: verySpecialSourceCodeUrl,
    description:
      'This is a very special bookmark used for testing the search functionality. Indeed very-special-bookmark',
    descriptionHtml:
      '<p>This is a very special bookmark used for testing the search functionality. Indeed very-special-bookmark</p>',
    userId: testUserId,
    public: isPublic,
    likeCount: 0,
    lastAccessedAt: null,
...
describe('Public API Tests', () => {
  describe('get public bookmarks root endpoint tests', () => {
    it('should return all public bookmarks', async () => {
      const response = await request(app).get(basePathApiPublicBookmarks);
    ...
    it('should not find bookmark by location', async () => {
      ...
      expect(response.statusCode).toEqual(HttpStatus.OK);
      expect(foundBookmarks).toHaveLength(0);
    });
  });
  ...
  describe('Test bookmarks search functionality', function () {
    ...
    const verySpecialTitle =
      'very special title very-special-java-title - public';
    const verySpecialLocation =
      'http://www.very-special-url.com/public-api-tests';
    const verySpecialTag = 'very-special-tag-public';
    const verySpecialSourceCodeUrl =
      'https://very-special-github-url.com/public-api-tests';
    let publicBookmarkExample = generateBookmark(
      verySpecialTitle,
      verySpecialLocation,
      verySpecialTag,
      verySpecialSourceCodeUrl,
    ...
    afterAll(async () => {
      await request(app)
        .delete(
          `${basePathApiPersonalUsers}${testUserId}/bookmarks/${createdBookmarkId}`
        )
        .set('Authorization', bearerToken);
    });
    ...
    it('should find bookmark tagged with very-special-tag', async function () {
      const response = await request(app).get(
        basePathApiPublicBookmarks + 'tagged/' + verySpecialTag
      );
