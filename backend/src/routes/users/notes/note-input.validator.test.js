...
const { NoteValidationErrorMessages, NoteValidationRules } = require('./note-input.validator');
...
describe('validateNoteInput', () => {
  ...
  test('does not throw an error when given valid input', () => {
    const validNote = { userId: 123, title: 'Test', content: 'This is a test note' };
    expect(() => noteInputValidator.validateNoteInput(validNote.userId, validNote)).not.toThrowError();
  });
});
