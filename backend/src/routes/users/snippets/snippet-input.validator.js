...
let validateSnippetInput = function(userId, snippet) {
  ...
  if (snippet.userId !== userId) {
    validationErrorMessages.push(SnippetValidationMessages.USER_ID_NOT_MATCHING);
  }
  ...
  if (!snippet.codeSnippets) {
    validationErrorMessages.push(SnippetValidationMessages.MISSING_CODE_SNIPPETS);
  }
  ...
  if(validationErrorMessages.length > 0){
    throw new ValidationError(SnippetValidationMessages.SNIPPET_NOT_VALID, validationErrorMessages);
  }
}
