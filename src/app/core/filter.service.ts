...
export class BookmarkFilterService {
    ...
    private bookmarkContainsSearchedTerm(codingmark: Codingmark, searchedTerm: string): boolean {
        let result = false;
        const escapedSearchPattern = '\\b' + this.escapeRegExp(searchedTerm.toLowerCase()) + '\\b'
        // const escapedSearchPattern = this.escapeRegExp(searchedTerm.toLowerCase()) + '[^\\w]'; //not working java javascript
