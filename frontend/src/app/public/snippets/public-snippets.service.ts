...
export class PublicSnippetsService {
  ...
  getRecentPublicSnippets(page: number, limit: number): Observable<Snippet[]> {
    ...
    return this.httpClient.get<Snippet[]>(this.publicSnippetsApiBaseUrl, {
      params: params,
    });
  }
  ...
  searchPublicSnippets(
    ...
    return this.httpClient
      .get<Bookmark[]>(this.publicSnippetsApiBaseUrl, { params: params })
      .pipe(shareReplay(1));
  }

  getPublicSnippetById(snippetId: string): Observable<Snippet> {
    return this.httpClient
      .get<Snippet>(`${this.publicSnippetsApiBaseUrl}/${snippetId}`)
      .pipe(
        shareReplay(1),
        catchError(() => {
  ...
  getSharedSnippetBySharableId(shareableId: string): Observable<Snippet> {
    return this.httpClient.get<Snippet>(
      `${this.publicSnippetsApiBaseUrl}/shared/${shareableId}`
    );
  }
}
