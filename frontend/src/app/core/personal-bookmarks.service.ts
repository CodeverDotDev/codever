...
export class PersonalBookmarksService {
  ...
  getPersonalBookmarkById(
    userId: string,
    bookmarkId: string
  ): Observable<Bookmark> {
    return this.httpClient
      .get<Bookmark>(
        `${this.personalBookmarksApiBaseUrl}/${userId}/bookmarks/${bookmarkId}`
      )
      .pipe(shareReplay(1));
  }
  ...
  updateBookmark(bookmark: Bookmark): Observable<any> {
    return this.httpClient
      .put(
        `${this.personalBookmarksApiBaseUrl}/${bookmark.userId}/bookmarks/${bookmark._id}`,
  ...
  createOrGetShareableId(userId: string, bookmarkId: string): Observable<any> {
    return this.httpClient
      .get<string>(
        `${this.personalBookmarksApiBaseUrl}/${userId}/bookmarks/shareable/${bookmarkId}`
      )
      .pipe(shareReplay(1));
  }
  ...
  increaseOwnerVisitCount(bookmark: Bookmark) {
    return this.httpClient
      .post(
        `${this.personalBookmarksApiBaseUrl}/${bookmark.userId}/bookmarks/${bookmark._id}/owner-visits/inc`,
        {},
        { headers: this.headers }
      )
      .pipe(shareReplay(1));
  ...
  deletePrivateBookmarksForTag(userId: string, tag: string): Observable<any> {
    const params = new HttpParams().set('tag', tag).set('type', 'private');
    return this.httpClient
      .delete(`${this.personalBookmarksApiBaseUrl}/${userId}/bookmarks`, {
        headers: this.headers,
        params: params,
      })
      .pipe(shareReplay(1));
  ...
  updateDisplayNameInBookmarks(
    userId: string,
    displayName: string
  ): Observable<any> {
    return this.httpClient
      .patch(
        `${this.personalBookmarksApiBaseUrl}/${userId}/bookmarks/display-name`,
