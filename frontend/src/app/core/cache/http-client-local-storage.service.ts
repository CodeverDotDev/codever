...
export class HttpClientLocalStorageService {
  ...
  private httpCall<T>(verb: Verbs, options: HttpOptions): Observable<T> {
    const canBePlacedInLocalStorage = !options.isSensitive || (options.isSensitive && this.localStorageService.load(localStorageKeys.userLocalStorageConsent));
    ...
    if (canBePlacedInLocalStorage) {
      ...
      if (options.cacheHours > 0) {
        // Get data from cache
        const data = this.localStorageService.load(options.key ? options.key : options.url)
