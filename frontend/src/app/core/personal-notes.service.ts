...
export class PersonalNotesService {
  ...
  getPersonalNoteById(userId: string, noteId: string): Observable<Note> {
    return this.httpClient.get<Note>(`${this.personalNotesApiBaseUrl}/${userId}/notes/${noteId}`).pipe(shareReplay(1));
  }
  ...
  getSuggestedNoteTags(userId: String): Observable<string[]> {
    return this.httpClient.get<string[]>(`${this.personalNotesApiBaseUrl}/${userId}/notes/suggested-tags`)
      .pipe(shareReplay(1));
  }

  updateNote(note: Note): Observable<any> {
    return this.httpClient
      .put(`${this.personalNotesApiBaseUrl}/${note.userId}/notes/${note._id}`, JSON.stringify(note),
  ...
  createNote(userId: string, note: Note): Observable<any> {
    return this.httpClient
      .post(`${this.personalNotesApiBaseUrl}/${userId}/notes`, JSON.stringify(note), {
        headers: this.headers,
        observe: 'response'
      })
      .pipe(shareReplay(1));
  ...
  deleteNoteById(userId: string, noteId: string): Observable<any> {
    return this.httpClient
      .delete(`${this.personalNotesApiBaseUrl}/${userId}/notes/${noteId}`, {headers: this.headers})
      .pipe(shareReplay(1));
  }


  getFilteredPersonalNotes(searchText: string, limit: number, page: number, userId: string, include: string) {
