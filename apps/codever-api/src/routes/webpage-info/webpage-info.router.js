...
router.get(
  '/scrape',
  keycloak.protect('realm:ROLE_USER'),
  async function (request, response, next) {
    const location = request.query.location;
    if (location) {
      const webpageData = await WebPageInfoService.getScrapedDataForLocation(
        location
      );
...
router.get(
  ...
  async function (request, response, next) {
    ...
    if (youtubeVideoId) {
      const webpageData = await WebPageInfoService.getYoutubeVideoData(
        youtubeVideoId
      );
...
router.get(
  ...
  async function (request, response, next) {
    ...
    if (stackoverflowQuestionId) {
      const webpageData = await WebPageInfoService.getStackoverflowQuestionData(
