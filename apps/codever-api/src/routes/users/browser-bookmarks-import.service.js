...
let importBrowserBookmarks = async function (
  userId,
  bookmarksHtmlFileContent,
  userDisplayName
) {
    ...
    try {
      ...
      if (existingBookmark) {
        ...
        if (existingBookmark.public) {
          const sanitizeForPublic = mergedTagsUnique.filter(
            (tag) => tag !== IMPORTED_BROWSER_TAG
          );
...
function generateBookmarkFields(userId, $a, userDisplayName) {
  ...
  let loweredDashedCase = categories.map((category) =>
