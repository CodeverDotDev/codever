...
personalBookmarksRouter.post(
  ...
  async (request, response) => {
    ...
    let newBookmark = await PersonalBookmarksService.createBookmark(
...
personalBookmarksRouter.get(
  ...
  async (request, response, next) => {
    ...
    if (searchText) {
      const bookmarks =
        await personalBookmarksSearchService.findPersonalBookmarks(
          request.params.userId,
          searchText,
          page,
          limit,
...
personalBookmarksRouter.get(
  ...
  async (request, response, next) => {
    if (request.query.location) {
      ...
      const bookmark =
        await PersonalBookmarksService.getPersonalBookmarkByLocation(
          request.params.userId,
          decodedUrl
        );
...
personalBookmarksRouter.get(
  ...
  async (request, response) => {
    ...
    switch (orderBy) {
      case 'MOST_LIKES':
        bookmarks = await PersonalBookmarksService.getMostLikedBookmarks(
          request.params.userId,
          page,
          limit
        );
        break;
      case 'LAST_CREATED':
        bookmarks = await PersonalBookmarksService.getLastCreatedBookmarks(
          request.params.userId,
          page,
          limit
        );
      ...
      case 'MOST_USED':
        ...
        break;
      default:
        bookmarks = await PersonalBookmarksService.getAllMyBookmarks(
          request.params.userId
        );
    }
...
personalBookmarksRouter.get(
  ...
  async (request, response) => {
    UserIdValidator.validateUserId(request);
    const tags = await PersonalBookmarksService.getSuggestedTagsForUser(
      request.params.userId
    );
...
personalBookmarksRouter.get(
  '/tags',
  keycloak.protect(),
  NodeCache.cacheMiddleware(240),
  async (request, response) => {
    UserIdValidator.validateUserId(request);
    const tags = await PersonalBookmarksService.getUserTagsAggregated(
      request.params.userId
    );
...
personalBookmarksRouter.get(
  ...
  async (request, response) => {
    ...
    const bookmark = await PersonalBookmarksService.getBookmarkById(
      userId,
      bookmarkId
    );
...
personalBookmarksRouter.get(
  '/shareable/:bookmarkId',
  keycloak.protect(),
  async (request, response) => {
    UserIdValidator.validateUserId(request);

    const { userId, bookmarkId } = request.params;
    const shareableId = await PersonalBookmarksService.getOrCreateSharableId(
      userId,
      bookmarkId
    );
...
personalBookmarksRouter.put(
  ...
  async (request, response) => {
    ...
    const { userId, bookmarkId } = request.params;
    const updatedBookmark = await PersonalBookmarksService.updateBookmark(
      userId,
      bookmarkId,
      bookmark
    );
 ...
 */
personalBookmarksRouter.delete(
  '/:bookmarkId',
  keycloak.protect(),
  async (request, response) => {
    UserIdValidator.validateUserId(request);
...
personalBookmarksRouter.delete(
  ...
  async (request, response, next) => {
    ...
    if (location) {
      await PersonalBookmarksService.deleteBookmarkByLocation(
        request.params.userId,
        location
      );
      return response.status(HttpStatus.NO_CONTENT).send();
...
personalBookmarksRouter.delete(
  ...
  async (request, response, next) => {
    ...
    if (tag && type === 'private') {
      const deletedCount =
        await PersonalBookmarksService.deletePrivateBookmarksByTag(
          request.params.userId,
          tag
        );
      return response
        .status(HttpStatus.OK)
        .send({ deletedCount: deletedCount });
    } else {
      next();
    }
  }
...
personalBookmarksRouter.patch(
  ...
  async (request, response) => {
    ...
    await PersonalBookmarksService.updateDisplayNameInBookmarks(
      userId,
      displayName
    );
