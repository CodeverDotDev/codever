...
personalSnippetsRouter.post(
  ...
  async (request, response) => {
    ...
    response
      .set(
        'Location',
        `${config.basicApiUrl}/personal/users/${request.params.userId}/snippets/${newSnippet.id}`
      )
      .status(HttpStatus.CREATED)
      .send({
        response: 'Snippet created for userId ' + request.params.userId,
      });
  }
);
 ...
 **/
personalSnippetsRouter.get(
  '/suggested-tags',
  keycloak.protect(),
  async (request, response) => {
    UserIdValidator.validateUserId(request);
    const tags = await PersonalSnippetsService.getSuggestedSnippetTags(
      request.params.userId
    );
...
personalSnippetsRouter.get(
  ...
  async (request, response) => {
    UserIdValidator.validateUserId(request);
    const tags = await PersonalSnippetsService.getUserSnippetTags(
      request.params.userId
    );
...
personalSnippetsRouter.get(
  ...
  async (request, response) => {
    UserIdValidator.validateUserId(request);
    const snippets = await PersonalSnippetsService.getAllMySnippets(
      request.params.userId
    );
...
personalSnippetsRouter.get(
  ...
  async (request, response) => {
    ...
    const shareableId = await PersonalSnippetsService.getOrCreateShareableId(
      userId,
      snippetId
    );
...
personalSnippetsRouter.get(
  ...
  async (request, response, next) => {
    ...
    if (query) {
      const snippets = await SnippetsSearchService.findPersonalSnippets(
        userId,
        query,
        page,
        limit,
...
personalSnippetsRouter.get(
  ...
  async (request, response) => {
    ...
    const snippets = await PersonalSnippetsService.getLatestSnippets(
      userId,
      limit || 20
    );
...
personalSnippetsRouter.get(
  ...
  async (request, response) => {
    ...
    const snippet = await PersonalSnippetsService.getSnippetById(
      userId,
      snippetId
    );
...
personalSnippetsRouter.put(
  ...
  async (request, response) => {
    ...
    const { userId, snippetId } = request.params;
    const updatedSnippet = await PersonalSnippetsService.updateSnippet(
      userId,
      snippetId,
      snippetData
    );
