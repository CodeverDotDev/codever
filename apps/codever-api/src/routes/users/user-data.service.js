...
let updateUserDataHistory = async function (history, userId) {
  history = trimMaxAllowedStoreLength(
    history,
...
function validateUserData(userData, userId) {
  ...
  if (validationErrorMessages.length > 0) {
    throw new ValidationError(
      'Submitted user data is not valid',
...
let getReadLater = async function (userId, page, limit) {
  ...
  if (!userData) {
    ...
    const readLaterRangeIds = userData.readLater.slice(
      (page - 1) * limit,
      (page - 1) * limit + limit
    );
...
let getFavoriteBookmarks = async function (userId, page, limit) {
  ...
  if (!userData) {
    ...
    const favoritesRangeIds = userData.favorites.slice(
      (page - 1) * limit,
      (page - 1) * limit + limit
    );
    ...
    const orderedBookmarksAsInFavorites = bookmarks.sort(function (a, b) {
      return (
        favoritesRangeIds.indexOf(a._id) - favoritesRangeIds.indexOf(b._id)
      );
    });
...
let getBookmarksFromHistory = async function (userId, page, limit) {
  ...
  if (!userData) {
    ...
    const historyRangeIds = userData.history.slice(
      (page - 1) * limit,
      (page - 1) * limit + limit
    );
...
let validateInputForBookmarkRating = function (userData, userId) {
  ...
  if (userId !== userData.ratingUserId) {
    validationErrorMessages.push(
      'The ratingUserId in the request.body must be the same as the userId request parameter'
    );
  }
  if (!userData.action) {
  ...
  if (!(userData.action === 'LIKE' || userData.action === 'UNLIKE')) {
    validationErrorMessages.push(
      'Invalid value - rating action should be LIKE or UNLIKE'
    );
  }
  if (validationErrorMessages.length > 0) {
    throw new ValidationError(
      'Rating bookmark input is not valid',
      validationErrorMessages
    );
  }
};
...
let likeBookmark = async function (userData, userId, bookmarkId) {
  if (userData.likes.includes(bookmarkId)) {
    throw new ValidationError('You already starred this bookmark', [
    ...
    if (bookmarkNotFound) {
      throw new NotFoundError(
        'Bookmark with bookmark id ' + bookmarkId + ' not found'
      );
    } else {
      return bookmark;
    }
...
let unlikeBookmark = async function (userData, userId, bookmarkId) {
  if (!userData.likes.includes(bookmarkId)) {
    throw new ValidationError('You did not like this bookmark', [
...
let getFeedBookmarks = async function (userId, page, limit) {
  ...
  if (!userData) {
    ...
    if (bookmarks.length < limit) {
      ...
      const pageOtherPublicBookmarks =
        count === 0 ? page - 1 : Math.floor((intervalRight - count) / limit);
      const limitOtherPublicBookmarks =
        intervalRight - count < limit ? intervalRight - count : limit;
