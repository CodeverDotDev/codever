...
let validateBookmarkInput = function (userId, bookmark) {
  ...
  if (bookmark.userId !== userId) {
    validationErrorMessages.push(
      'The userId of the bookmark does not match the userId parameter'
    );
  }
...
function validateInputExceptUserId(bookmark) {
  ...
  if (blockedTags) {
    validationErrorMessages.push(
      'The following tags are blocked:' + blockedTags
    );
  }

  if (bookmark.description) {
    const descriptionIsTooLong =
      bookmark.description.length >
      BookmarkValidationRules.MAX_NUMBER_OF_CHARS_FOR_DESCRIPTION;
    ...
    const descriptionHasTooManyLines =
      bookmark.description.split('\n').length >
      BookmarkValidationRules.MAX_NUMBER_OF_LINES_FOR_DESCRIPTION;
...
let verifyPublicBookmarkExistenceOnCreation = async function (bookmark) {
  if (bookmark.public) {
    const existingBookmark = await Bookmark.findOne({
      ...
      .exec();
    if (existingBookmark) {
      throw new PublicBookmarkExistentError(
        `Create: A public bookmark with this location is already present - location: ${bookmark.location}`
      );
    }
  }
...
let verifyPublicBookmarkExistenceOnUpdate = async function (bookmark, userId) {
  if (bookmark.public) {
    ...
    if (existingBookmark) {
      throw new PublicBookmarkExistentError(
        `Update: A public bookmark with this location is already present - location: ${bookmark.location}`
      );
    }
  }
...
module.exports = {
  ...
  verifyPublicBookmarkExistenceOnCreation:
    verifyPublicBookmarkExistenceOnCreation,
